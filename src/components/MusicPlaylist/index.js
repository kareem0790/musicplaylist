...
class MusicPlaylist extends Component {
  ...
  onDeletePlaylist = id => {
    ...
    const filtered = playlist.filter(each => each.id !== id)
    this.setState({playlist: filtered})
  }

  noSongsFound = () => (
    <NoSongsFoundContainer>
      <NotFoundHeading>No Songs Found</NotFoundHeading>
    </NoSongsFoundContainer>
  )

  render() {
    const {playlist, inputValue} = this.state
    const searchList = playlist.filter(play =>
      play.name.toLowerCase().includes(inputValue.toLowerCase()),
    )
